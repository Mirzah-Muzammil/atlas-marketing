...
export function DispatchChapters() {
  return (
    <section className="relative overflow-hidden bg-ink text-paper" id="dispatches">
      <div className="container-shell flex h-auto flex-col py-20 lg:h-screen lg:min-h-[720px] lg:justify-center lg:py-0">
        ...
        <div className="mt-10 flex flex-col gap-5 lg:w-max lg:flex-row" data-dispatch-track>
          {journeyStages.map((stage, index) => <article className="group relative min-h-[470px] overflow-hidden border border-paper/20 p-7 lg:w-[38vw] lg:max-w-[540px]" key={stage.id}><div className="absolute inset-x-0 bottom-0 h-1 origin-left scale-x-0 bg-accent transition-transform duration-700 group-hover:scale-x-100" /><p className="text-xs font-bold tracking-[0.17em] text-secondary">Dispatch 0{index + 1}</p><div className="mt-20 flex items-end justify-between border-b border-paper/20 pb-5"><h3 className="font-editorial text-6xl italic md:text-8xl">{stage.title}</h3><span className="text-5xl text-paper/10">{stage.number}</span></div><p className="mt-7 text-sm font-bold tracking-[0.14em] text-accent">{stage.eyebrow.toUpperCase()}</p><p className="mt-4 max-w-md text-base leading-7 text-paper/62">{stage.description}</p><p className="absolute bottom-7 left-7 font-editorial text-lg italic text-paper/85">“{stage.promise}”</p></article>)}
        </div>
      </div>
